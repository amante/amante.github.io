...
function App(){
  const [kind, setKind] = useState("rushing");
  ...
  const adjDefense = React.useMemo(()=>opponentAdjustment(Number(defenseAllow), Number(leagueMean), Number(elasticity)), [defenseAllow, leagueMean, elasticity]);
  const adjTemp = React.useMemo(()=>tempAdjustment(Number(tempC), {kind}), [tempC, kind]);
  const adjTime = React.useMemo(()=>timeAdjustment(Number(hourLocal)), [hourLocal]);
  const mu = React.useMemo(()=> muSurface * adjDefense * adjTemp * adjTime, [muSurface, adjDefense, adjTemp, adjTime]);
  const sd = React.useMemo(()=> estimateSD(Number(minY), Number(maxY)), [minY, maxY]);
  ...
  const tableData = React.useMemo(()=> thresholds.map(t=>({threshold:t, probability: probGE(t, mu, sd)})), [thresholds, mu, sd]);
  ...
  return (
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      <header className="px-6 py-6 border-b bg-white sticky top-0 z-10">
        ...
        <p className="text-sm text-neutral-500 mt-1">Ajustes por defensa, superficie, temperatura y horario.</p>
      </header>
      ...
      <main className="p-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="border rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-neutral-50">
                  <tr><th className="text-left px-3 py-2">Umbral (yd)</th><th className="text-right px-3 py-2">P(Y ≥ X)</th></tr>
                </thead>
                <tbody>
                  {tableData.map(r=>(
        ...
        <section className="lg:col-span-4 bg-white border rounded-2xl p-5 shadow-sm">
          ...
          <p className="text-xs text-neutral-500 mt-3">Modelo: Normal(μ, σ) con μ ajustada por defensa, superficie, temperatura y horario. σ ≈ (máx − mín)/4.</p>
        </section>
      </main>
